...
function Footer() { return <footer className="footer"><div className="container footer-grid"><div><span className="brand"><img src={brandIcon} alt="" className="brand-icon" loading="lazy" /><span className="wordmark">{site.doctor}</span></span><p>{site.specialty}</p><span>{CRM} · {RQE}</span></div><div><strong>{site.footer.contactTitle}</strong><a href={`mailto:${EMAIL}`}>{EMAIL}</a><a href={INSTAGRAM} target="_blank" rel="noreferrer"><Instagram />{site.footer.instagram}</a></div><div><strong>{site.footer.linksTitle}</strong>{site.nav.map(([label, href]) => <a key={href} href={href}>{label}</a>)}</div></div><address className="container footer-legal"><span>{BUSINESS.legalName} · CNPJ {BUSINESS.cnpj}</span><span>{BUSINESS.address} · <a href={`mailto:${EMAIL}`}>{EMAIL}</a></span></address><div className="container footer-bottom"><span>{site.footer.credit}</span><span>{site.footer.copyright}</span></div></footer>; }
...
function StructuredData() {
  ...
  const physician = {
    ...
    knowsAbout,
    // Pessoa jurídica do atendimento: casa o site com o cadastro da empresa na verificação da Meta.
    parentOrganization: {
      "@type": "Organization", name: BUSINESS.legalName, legalName: BUSINESS.legalName, taxID: BUSINESS.cnpj, email: EMAIL, url: SITE_URL,
      address: { "@type": "PostalAddress", streetAddress: BUSINESS.street, addressLocality: `${BUSINESS.district}, ${BUSINESS.city}`, addressRegion: BUSINESS.state, postalCode: BUSINESS.postalCode, addressCountry: "BR" },
    },
